...
$judge_plugins = get_list_of_plugins('mod/onlinejudge/judge');
foreach ($judge_plugins as $dir) {
    require_once("$CFG->dirroot/mod/onlinejudge/judge/$dir/lib.php");
}
...
function onlinejudge_get_judge_classes() {
    ...
    if (empty($judgeclasses)) {
        if ($plugins = get_list_of_plugins('mod/onlinejudge/judge')) {
