...
require_once($CFG->dirroot.'/mod/onlinejudge/moodle2/assignment.class.php');
...
class assignment_onlinejudge extends assignment_upload {
    ...
    function assignment_onlinejudge($cmid='staticonly', $assignment=NULL, $cm=NULL, $course=NULL) {
        ...
        if (isset($this->assignment->id)) {
            $this->onlinejudge = $DB->get_record('onlinejudge', array('id' => $this->assignment->id), '*', MUST_EXIST);
        }
    }
    ...
    function setup_elements(&$mform ) {
        ...
        if (!empty($update)) {
            ...
            $onlinejudge = $DB->get_record('onlinejudge', array('assignment' => $cm->instance));
        }
    ...
    function add_instance($assignment) {
        ...
        if ($assignment->id) {
            ...
            $DB->insert_record('onlinejudge', $onlinejudge);
        }
    ...
    function update_instance($assignment) {
        ...
        if ($returnid) {
            $onlinejudge = $assignment;
            $old_onlinejudge = $DB->get_record('onlinejudge', array('assignment' => $assignment->id));
            if ($old_onlinejudge) {
    ...
    function get_testcases() {
        ...
        $records = $DB->get_records('onlinejudge_testcases', array('assignment' => $this->assignment->id), 'sortorder ASC');
        $tests = array();
    ...
    function upload_file($mform, $options) {
        ...
        $returnurl  = new moodle_url('/mod/onlinejudge/view.php', array('id'=>$this->cm->id));
        $submission = $this->get_submission($USER->id);

        // block住了，暂时注释掉
        // if (!$this->can_upload_file($submission)) {
        //     $this->view_header(get_string('upload'));
        //     echo $OUTPUT->notification(get_string('uploaderror', 'onlinejudge'));
        ...
        if ($formdata = $mform->get_data()) {
            ...
            $files = $fs->get_area_files($this->context->id, 'user', 'draft', $submission->id);
    ...
    function get_onlinejudge_result($submission) {
        ...
        $sql = 'SELECT s.*, t.feedback, t.subgrade
                FROM {onlinejudge_submissions} s LEFT JOIN {onlinejudge_testcases} t
                ON s.testcase = t.id
    ...
    function update_submission($submission, $new_oj=false) {
        ...
        if ($new_oj) {
            ...
            $DB->insert_record('onlinejudge_submissions', $submission);
        } else {
            $submission->id = $submission->oj_id;
            $DB->update_record('onlinejudge_submissions', $submission);
        }
    }
    ...
    function request_judge($submission) {
        ...
        $oj = $DB->get_record('onlinejudge', array('id' => $submission->assignment), '*', MUST_EXIST);
        ...
        $DB->set_field('onlinejudge_submissions', 'latest', 0, array('submission' => $submission->id));
        ...
        foreach ($tests as $test) {
            ...
            try {
                ...
                $DB->insert_record('onlinejudge_submissions', array('submission' => $submission->id, 'testcase' => $test->id, 'task' => $taskid, 'latest' => 1));
...
function onlinejudge_task_judged($task) {
    ...
    $sql = 'SELECT s.*
        FROM {assignment_submissions} s LEFT JOIN {onlinejudge_submissions} o
